...
def generate_excel_report(data: dict, output_path: str = None) -> str:
    ...
    for item in table1:
        ...
        if field in [
            ...
            tech_params[field] = value

    if tech_params:
        tech_row_start = 10

        ws[f"D{tech_row_start}"] = "TECHNICAL PARAMETERS:"
        ws[f"D{tech_row_start}"].font = Font(bold=True, size=12)
        ...
        tech_labels = {
            ...
            "PRESSURE_TEST_BODY": "Pressure Test (Body):",
            "PRESSURE_TEST_SEAT": "Pressure Test (Seat):",
        }

        current_tech_row = tech_row_start + 1
        for field, label in tech_labels.items():
            if field in tech_params and tech_params[field]:
                ws[f"D{current_tech_row}"] = label
                ws[f"E{current_tech_row}"] = tech_params[field]
                ws[f"D{current_tech_row}"].font = Font(bold=True)
                current_tech_row += 1
    ...
    header_row = 18  # Сдвинули вниз чтобы вместить технические параметры
